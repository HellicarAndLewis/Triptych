...
void testApp::update(){
	...
	if(kinect.isFrameNew()) {
		...
		int numPix = kinect.getWidth()*kinect.getHeight();
		for(int i = 0; i < numPix; i++) {
		
			if(pix[i]>nearThreshold) {
				
				pix[i] = 0;
			} else if(pix[i]<farThreshold) {
				
				pix[i] = 0;
			} else {
					
				float f = (pix[i] - farThreshold)*255.f/(nearThreshold-farThreshold);
				pix[i] = f;
			}
		}
		grey.setFromPixels(pix, kinect.getWidth(), kinect.getHeight());
		contours.findContours(grey, 30*30, 480*480, 20, false);
