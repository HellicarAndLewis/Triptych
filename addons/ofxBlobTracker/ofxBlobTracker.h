...
class ofxBlobEventLister: public ofxBlobListener {
public:	
	void clear() {
		list.clear();
	}
	...
	void blobEntered(ofVec3f pos, int blobId) {
		list.push_front(ofxBlobEvent(pos, blobId, ofxBlobTracker_entered));
	}
	
	void blobMoved(ofVec3f pos, int blobId) {
		list.push_front(ofxBlobEvent(pos, blobId, ofxBlobTracker_moved));
	}
	
	void blobExited(ofVec3f pos, int blobId) {
		list.push_front(ofxBlobEvent(pos, blobId, ofxBlobTracker_exited));
	}
	
	bool getNextEvent(ofxBlobEvent &e) {
		if(list.size()>0) {
			e.pos = list.back().pos;
			e.blobId = list.back().blobId;
			e.eventType = list.back().eventType;
			list.pop_back();
			return true;
		}
