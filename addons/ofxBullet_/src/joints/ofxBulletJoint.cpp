...
void ofxBulletJoint::_setDefaults() {
	...
	_joint->setParam(BT_CONSTRAINT_STOP_ERP,0.05,0);
	_joint->setParam(BT_CONSTRAINT_STOP_ERP,0.05,1);
	_joint->setParam(BT_CONSTRAINT_STOP_ERP,0.05,2);
	_joint->setParam(BT_CONSTRAINT_STOP_ERP,0.05,3);
	_joint->setParam(BT_CONSTRAINT_STOP_ERP,0.05,4);
	_joint->setParam(BT_CONSTRAINT_STOP_ERP,0.05,5);
}

void ofxBulletJoint::setParam(int param, float value) {
	for (int i = 0; i < 6; i++) {
		_joint->setParam(param, value, i);
	}
}
